...
<?php echo "\t<?php if (!empty(\$" . $pluralVar . ")): ?>\n";?>
	...
	<table cellpadding="0" cellspacing="0" class="<?php echo $pluralVar;?>  table table-bordered table-striped">
		<thead>
			<tr>
<?php foreach ($fields as $field): if (in_array($field, array('deleted', 'tenant_id', 'gmt_created', 'gmt_modified')) === false): ?>
...
<?php echo "\t\t\t\t<th class=\"actions\"><?php echo __('Actions');?></th>\n";?>
		...
		echo "\t\t\t<tr>\n";
			foreach ($fields as $field) {
				if (in_array($field, array('deleted', 'tenant_id', 'gmt_created', 'gmt_modified')) !== false) {
					continue;
				}
				$isKey = false;
			...
			echo "\t\t\t\t\t<?php echo \$this->Html->link('<i class=\"icon-repeat\"></i> ' . __('Recycle'), array('action' => 'recycle', \${$singularVar}['{$modelClass}']['{$primaryKey}']), array('escape' => false, 'class' => 'btn btn-mini')); ?>\n";
			echo "\t\t\t\t\t<?php echo \$this->Html->link('<i class=\"icon-remove\"></i> ' . __('Drop'), array('action' => 'drop', \${$singularVar}['{$modelClass}']['{$primaryKey}']), array('escape' => false, 'class' => 'btn btn-mini')); ?>\n";
			echo "\t\t\t\t</td>\n";
		echo "\t\t\t</tr>\n";
