	...
	<table cellpadding="0" cellspacing="0" class="<?php echo $inflector['underscore'];?> table table-bordered table-striped">
		...
		<?php $i = 0; foreach (${$inflector['plural']} as $post): ?>
			<tr>
				...
				<td><?php echo h($post[$inflector['camelize']]['gmt_deleted']); ?>&nbsp;</td>
				<td class="actions">
					<?php echo $this->Html->link('<i class="icon-repeat"></i> ' . __('Recycle'), array('action' => 'recycle', $post[$inflector['camelize']]['id']), array('escape' => false, 'class' => 'btn btn-mini')); ?>
					<?php echo $this->Html->link('<i class="icon-remove"></i> ' . __('Drop'), array('action' => 'drop', $post[$inflector['camelize']]['id']), array('escape' => false, 'class' => 'btn btn-mini')); ?>
				</td>
			</tr>
		<?php endforeach; ?>
		</tbody>
