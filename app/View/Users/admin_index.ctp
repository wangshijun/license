<div class="users index">
	...
	<?php echo $this->BootstrapForm->create('User', array('url' => array_merge(array('action' => 'search'), $this->params['pass']), 'class' => 'search form-search form-inline'));?>
	...
	<table cellpadding="0" cellspacing="0" class="users  table table-bordered table-striped">
		...
		<?php $i = 0; foreach ($users as $user): ?>
			<tr>
				...
				<td class="actions">
					<?php echo $this->Html->link('<i class="icon-refresh"></i> ' . __('Reset Password'), array('action' => 'reset_password', $user['User']['id']), array('escape' => false, 'class' => 'btn btn-mini')); ?>
					...
					<?php echo $this->BootstrapForm->postLink('<i class="icon-trash"></i> ' . __('Delete'), array('action' => 'delete', $user['User']['id']), array('escape' => false, 'class' => 'btn btn-mini'), __('Are you sure you want to delete # %s?', $user['User']['id'])); ?>
				</td>
			</tr>
		<?php endforeach; ?>
		</tbody>
